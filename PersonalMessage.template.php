...
function template_pm_avatar($avatar ='', $size ='',$class= '',$return = false){
	...
	else
	{
		foreach ($context['unread_pms'] as $id_pm => $pm_details)
			...
			<div class="d-flex p-3 bg-secondary" style="margin: 0 -11px;">
				...
				if(!empty($pm_details['member']))
						template_pm_avatar( $pm_details['member']['avatar']['image'] ,'32','rounded-circle me-2');
				 
					echo'
				</div>
				<div >
					<strong class="subject">',
					!empty($pm_details['member']) ? $pm_details['member']['link'] : $pm_details['member_from'], '  </strong>
					', $pm_details['pm_link'], '
					<div class="sender d-flex">
						', $pm_details['replied_to_you'] ? '<span class="main_icons replied centericon" title="' . $txt['pm_you_were_replied_to'] . '"></span>' : '<span class="main_icons im_off centericon"  title="' . $txt['pm_was_sent_to_you'] . '"></span>

						<small class="d-block"> ', $pm_details['time'], '</small>
					</div>
				</div>
			</div>';
	}
...
function template_single_pm($message)
{
	...
	if (!empty($modSettings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image']))
		echo '
					<li class="avatar">
						<a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '">', template_pm_avatar($message['member']['avatar']['image'],'32'), '</a>
					</li>';
