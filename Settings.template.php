...
function template_settings()
{
	...
	$customboardid = !empty($settings['spirate_news_board'])? (int) $settings['spirate_news_board']: 0;
	$context['theme_settings'] = array(
		$txt['spirtate_theme'],
		
		array(
			'id' => 'spirate_brand_color',
			'label' => $txt['spirate_brand_color'], 
			'description' =>  $txt['spirate_brand_desc'],
			'type' => 'color',
		),
		array(
			'id' => 'spirate_color',
			'label' => $txt['spirate_color'],
			'description' =>  $txt['spirate_color_desc'],
			'type' => 'list',
			'options' => array( 
				0 => $txt['spirate_color_light'], 
				1 => $txt['spirate_color_dark'],
			),
		), 
		array(
			'id' => 'spirate_index',
			'label' => $txt['spirate_index'],
			'description' =>  $txt['spirate_index_desc'],
			'type' => 'list',
			'options' => array(
				0 => $txt['spirate_index_recent'],
				1 => $txt['spirate_index_boardnews'], 
			),
		),
		array(
			'id' => 'spirate_news_board',
			'label' => $txt['spirate_news_board'],
			'description' =>  $txt['spirate_news_board_desc'],
			'type' => 'list', 
			'options' => array(
				$customboardid => $txt['spirate_news_board'], 
			),
		),
		array(
			'id' => 'spirtate_analytics',
			'label' => $txt['spirtate_analytics'], 
			'description' =>  $txt['spirtate_analytics_desc'],
			'type' => 'text',
		),
		array(
			'id' => 'spirtate_fluid_container',
			'label' => $txt['spirtate_fluid_container'], 
			'description' =>  $txt['spirtate_fluid_container_desc'],
			'type' => 'checkbox',
		),
		'',
		array(
