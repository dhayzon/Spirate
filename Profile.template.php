...
function template_profile_popup()
{
	...
	echo'  <h6 class="dropdown-header">', $context['member']['group'], '</h6> ';
	echo '  
		<div class="profile_user_links px-2">
			<div>';
...
function template_alerts_popup()
{
	...
	// Unlike almost every other template, this is designed to be included into the HTML directly via $().load()
	echo '<div class="w-100 ps-3">
		<div>
			<div class="alerts_opts block pb-2">
		...
		<div style="border-bottom: 1px solid var(--smf-border);	margin:0 -13px"></div>
	...
	else
	{
		...
		{
			echo '
			<', !$details['show_links'] ? 'a href="' . $scripturl . '?action=profile;area=showalerts;alert=' . $id_alert . '" onclick="this.classList.add(\'alert_read\')"' : 'div', ' class="py-2 mb-2">
			 <div class="d-flex p-3 bg-secondary" style="margin: 0 -11px;">
				<div class="float-start">';
						if(!empty($details['sender']['avatar']['image']))
						 template_profile_avatar($details['sender']['avatar']['image'],'32','rounded-circle me-2');
				echo'  
				</div> 
				<div class="alert_text">', $details['icon'], ' ', $details['text'], '<small class="d-block">', $details['time'], '</small></div> 
				 
				
			</div>
			</', !$details['show_links'] ? 'a' : 'div', '><div style="border-bottom: 1px solid var(--smf-border);	margin:0 -13px"></div>';
		}
	}
...
function template_profile_avatar($avatar ='', $size ='',$class= '',$return = false){
