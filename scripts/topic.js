...
QuickModify.prototype.onModifyDone = function (XMLDoc)
{
	...
	if (body)
	{
		...
		console.log(this.sCurMessageId);
...
$(function() {
	...
	$(document).on('click', '.msg_like', function(event){
		...
		var ele = obj.parent().find('.counter');
		
		__SMFGET(obj.attr('href') + ';js=1')
		.then(data => { 
			
			if(data.result.success){
				let check= parent.find('.check').remove(); 
				ele.text(data.result.data.count)
				ele.after(data.result.data.already)
			}else{ 
				reqOverlayDiv(smf_scripturl+'?action=login', help_popup_heading_text , '')
			}
		});
...
async function __SMFGET(url = '', data = {}) {
